...
function searchBuildings() {
    ...
    // 디버깅: 첫 번째 빌딩 데이터 구조 확인
    if (buildingsData && buildingsData.length > 0) {
        console.log('첫 번째 빌딩 데이터 예시:', buildingsData[0]);
        console.log('빌딩의 모든 속성:', Object.keys(buildingsData[0]));
    }
    ...
    const filtered = buildingsData.filter(building => {
        ...
        if (areaFrom > 0 || areaTo < Infinity) {
            console.log(`빌딩 "${building.name}"의 면적 관련 필드:`, {
                typicalFloorArea: building.typicalFloorArea,
                기준층전용면적: building.기준층전용면적,
                기준층면적: building.기준층면적,
                전용면적: building.전용면적,
                연면적: building.연면적
            });
        }
        ...
        for (const fieldName of possibleAreaFields) {
            if (building[fieldName]) {
                const fieldValue = building[fieldName].toString();
                const extracted = parseFloat(fieldValue.replace(/[^0-9.]/g, ''));
                if (!isNaN(extracted)) {
                    areaValue = extracted;
                    break;
                }
        ...
        if (areaFrom > 0 || areaTo < Infinity) {
            if (areaValue !== null) {
                ...
                if (!areaMatch) {
                    console.log(`빌딩 "${building.name}"는 면적 조건 불만족: ${areaValue}평`);
                }
            } else {
                // 면적 검색 조건이 있는데 건물에 면적 데이터가 없으면 제외
                areaMatch = false;
                console.log(`빌딩 "${building.name}"는 면적 데이터 없음`);
            }
        }
