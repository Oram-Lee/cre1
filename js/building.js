...
function displaySelectedBuildingsList() {
    ...
    // selectedBuildings가 정의되지 않았으면 빈 배열로 처리
    if (typeof selectedBuildings === 'undefined' || !selectedBuildings) {
        container.innerHTML = `
            <div class="empty-state">
                <div class="empty-state-icon">📋</div>
...
function updateTabCounts() {
    ...
    if (listCountEl) {
        const listCount = (typeof currentDisplayedBuildings !== 'undefined' && currentDisplayedBuildings) 
            ? currentDisplayedBuildings.length 
            : 0;
    ...
    if (selectedCountEl) {
        const selectedCount = (typeof selectedBuildings !== 'undefined' && selectedBuildings) 
