...
async function exportToExcel() {
    ...
    try {
        ...
        worksheet.mergeCells('B6:C6');  // 빈 셀
        ...
        setCategoryCell(worksheet, 'B24', '※ 정확한 제안을 원하시면 하기 사항 수기 부탁드립니다', 'FFFFFFFF', true);
        setCategoryCell(worksheet, 'B25', '임차 제안', 'FFF9D6AE');
        setCategoryCell(worksheet, 'B32', '임대 기준', 'FFD9ECF2');
        setCategoryCell(worksheet, 'B40', '임대 기준 조정', 'FFD9ECF2');
        ...
        const cColumnData = {
            ...
            15: '기준층 전용면적 (m²)',
            16: '기준층 전용면적 (평)',
            17: '엘레베이터',  // 주차 대수 정보와 위치 교환
            18: '냉난방 방식',
            19: '건물용도',
            20: '구조',
            21: '주차 대수 정보',  // 엘레베이터와 위치 교환
            22: '주차비',  // 주차 운영 삭제하고 주차비를 위로 이동
            23: '주차 대수',
            // 24행은 빈칸으로 유지
            25: '최적 임차 층수',  // 한 행 위로 이동
            26: '입주 가능 시기',  // 한 행 위로 이동
            27: '거래유형',  // 한 행 위로 이동
            28: '임대면적 (m²)',  // 한 행 위로 이동
            29: '전용면적 (m²)',  // 신규 추가
            30: '임대면적 (평)',
        ...
        worksheet.getCell('B52').font = { name: '맑은 고딕', size: 10, bold: true };
        ...
        [53, 54, 55].forEach(row => {
            worksheet.getCell(`B${row}`).font = { name: '맑은 고딕', size: 10 };
...
function fillBuildingData(worksheet, building, col) {
    ...
    // 빌딩 세부현황
    setDataCell(worksheet, `${col}17`, building.elevator || '');  // 엘레베이터 (17행으로 이동)
    ...
    setDataCell(worksheet, `${col}21`, building.parkingSpace || '');  // 주차 대수 정보 (21행으로 이동)
    
    // 주차 관련
    setDataCell(worksheet, `${col}22`, building.parkingFee || '');  // 주차비 (22행으로 이동)
    ...
    setNumericCell(worksheet, `${col}28`, 0, '#,##0.000 "m²"');
    setNumericCell(worksheet, `${col}29`, 0, '#,##0.000 "m²"');  // 전용면적 (m²) 신규 추가
    setNumericCell(worksheet, `${col}30`, 0, '#,##0.000 "평"');
    setNumericCell(worksheet, `${col}31`, 0, '#,##0.000 "평"');
    
    // 임대 기준 (기본값 0)
    for (let row = 32; row <= 39; row++) {
        setNumericCell(worksheet, `${col}${row}`, 0, '₩#,##0', 'right');
    }
