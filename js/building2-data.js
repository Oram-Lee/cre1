...
// 빌딩 데이터를 워크시트에 입력 (LG 버전 - 최대 10개 지원)
function fillBuildingDataLG(worksheet, building, buildingIndex) {
    if (buildingIndex >= 10) return; // 최대 10개
...
// 빌딩 데이터 일괄 입력 (최대 10개)
function fillAllBuildingDataLG(worksheet, buildings) {
    buildings.forEach((building, index) => {
        if (index < 10) { // 최대 10개
            fillBuildingDataLG(worksheet, building, index);
        }
    });
}

// 빌딩 데이터 검증
function validateBuildingDataLG(worksheet, buildingIndex) {
    const startCol = LG_TEMPLATE_CONFIG.buildingColumns[buildingIndex];
    const errors = [];
    
    // 필수 데이터 체크
    const requiredCells = [18, 19, 20]; // 주소, 위치, 준공일
    requiredCells.forEach(row => {
        const cell = worksheet.getCell(`${startCol}${row}`);
        if (!cell.value) {
            errors.push(`${startCol}${row}: 필수 데이터가 누락되었습니다.`);
        }
    });
...
// 대량 데이터 처리를 위한 배치 함수
function batchFillBuildingDataLG(worksheet, buildings) {
    console.log(`LG 데이터 입력 시작: ${buildings.length}개 빌딩`);
    
    buildings.forEach((building, index) => {
        if (index < 10) {
            console.log(`빌딩 ${index + 1} 데이터 입력: ${building.name}`);
            fillBuildingDataLG(worksheet, building, index);
        }
    });
    
    console.log('LG 데이터 입력 완료');
}

// 전역 함수로 등록
window.fillBuildingDataLG = fillBuildingDataLG;
window.fillAllBuildingDataLG = fillAllBuildingDataLG;
window.validateBuildingDataLG = validateBuildingDataLG;
window.batchFillBuildingDataLG = batchFillBuildingDataLG;