...
// 열 너비 최적화 함수
function getOptimizedColumnWidth(buildingCount) {
    if (buildingCount <= 5) {
        return 26.33203125; // 기존 너비
...
// ===== ExcelJS를 사용한 엑셀 내보내기 - 10개 지원 버전 =====
async function exportToExcel() {
    ...
    try {
        ...
        // 1. 열 너비 설정 - 동적으로 조정
        const buildingColumnWidth = getOptimizedColumnWidth(selectedBuildings.length);
        const columns = [
            { width: 2.6640625 },   // A열
            { width: 13.21875 },    // B열
            { width: 24.5546875 },  // C열
        ...
        for (let i = 0; i < selectedBuildings.length; i++) {
            columns.push({ width: buildingColumnWidth });
        }
        ...
        // 5. 빌딩 데이터 입력 (D열부터 최대 M열까지)
        selectedBuildings.forEach((building, index) => {
            const col = String.fromCharCode(68 + index); // D, E, F, G, H, I, J, K, L, M
