...
async function exportToExcel() {
    ...
    try {
        ...
        Object.entries(cColumnData).forEach(([row, value]) => {
            ...
            cell.alignment = { 
                horizontal: centerAlignRows.includes(parseInt(row)) ? 'center' : 'left', 
                vertical: 'middle' 
            };
            cell.border = {
                top: { style: 'thin' },
...
function fillBuildingData(worksheet, building, col) {
    ...
    // 빌딩 세부현황
    setDataCell(worksheet, `${col}17`, building.parkingSpace || '');
    ...
    setDataCell(worksheet, `${col}21`, building.elevator || '');
    
    // 주차 관련
    setDataCell(worksheet, `${col}22`, building.parkingOperation || '');
    setDataCell(worksheet, `${col}23`, building.parkingSpace || '');
    setDataCell(worksheet, `${col}24`, building.parkingFee || '');
    
    // 임차 제안 (기본값)
    setDataCell(worksheet, `${col}26`, '-');
    setDataCell(worksheet, `${col}27`, '-');
    setDataCell(worksheet, `${col}28`, '-');
    setNumericCell(worksheet, `${col}29`, 0, '#,##0.000 "m²"');
