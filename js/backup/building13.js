...
async function exportToExcel() {
    ...
    try {
        ...
        // 3. 셀 병합
        worksheet.mergeCells('B3:C4');  // PRESENT TO
        worksheet.mergeCells('B5:C5');  // 로고 영역
        worksheet.mergeCells('B6:C6');  // 빌딩개요/일반
        worksheet.mergeCells('B7:B18'); // 빌딩 현황
        worksheet.mergeCells('B19:B20'); // 빌딩 세부현황
        worksheet.mergeCells('B21:B23'); // 주차 관련
        worksheet.mergeCells('B25:B31'); // 임차 제안
        worksheet.mergeCells('B32:B39'); // 임대 기준
        worksheet.mergeCells('B40:B44'); // 임대 기준 조정
        ...
        setCategoryCell(worksheet, 'B24', '**항은색 최종 지정시 항은색 안내문구 식재 필수', 'FFFFFFFF', true);
        setCategoryCell(worksheet, 'B25', '임차 제안', 'FFF9D6AE');
        
        // 임차 제안 내부 빨간색 텍스트 (B28 위치에 작성)
        const b28 = worksheet.getCell('B28');
        b28.value = '(평기준 임대/전용면적 입력시 자동계산)';
        b28.font = { name: 'Noto Sans KR', size: 9, bold: true, color: { argb: 'FFFF0000' } };
        b28.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF9D6AE' } };
        b28.alignment = { horizontal: 'center', vertical: 'bottom' };
        
        setCategoryCell(worksheet, 'B32', '임대 기준', 'FFD9ECF2');
        
        // 임대 기준 내부 빨간색 텍스트 (B36 위치에 작성)
        const b36 = worksheet.getCell('B36');
        b36.value = '(평기준 보/임/관 입력시 자동계산)';
        b36.font = { name: 'Noto Sans KR', size: 9, bold: true, color: { argb: 'FFFF0000' } };
        b36.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD9ECF2' } };
        b36.alignment = { horizontal: 'center', vertical: 'bottom' };
        
        setCategoryCell(worksheet, 'B40', '임대기준 조정', 'FFD9ECF2');
        
        // 임대기준 조정 내부 빨간색 텍스트 (B42 위치에 작성)
        const b42 = worksheet.getCell('B42');
        b42.value = '(렌트프리 개월 입력시 자동계산)';
        b42.font = { name: 'Noto Sans KR', size: 9, bold: true, color: { argb: 'FFFF0000' } };
        b42.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD9ECF2' } };
        b42.alignment = { horizontal: 'center', vertical: 'bottom' };
...
function fillBuildingData(worksheet, building, col) {
    ...
    // 30행, 31행: 평 단위 (사용자 입력, 기본값 0)
    setNumericCell(worksheet, `${col}30`, 0, '#,##0.000 "평"');
    setNumericCell(worksheet, `${col}31`, 0, '#,##0.000 "평"');
    ...
    worksheet.getCell(`${col}39`).value = { formula: `(${col}37+${col}38)/${col}31` };
    ...
    worksheet.getCell(`${col}44`).value = { formula: `((${col}42+${col}43)*(${col}30/${col}31))` };
